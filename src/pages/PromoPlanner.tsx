...
import { CheckCircle, Loader2, ArrowRight, Calendar, FileText, Sun, Rocket } from "lucide-react";
...
const BENEFITS = [
  { icon: Calendar, title: "Monthly Promo Calendar", desc: "A ready-made promotion plan for the upcoming month, delivered on the 1st." },
  { icon: FileText, title: "Social Copy Included", desc: "Every promo comes with captions, hashtags, and email subject lines." },
  { icon: Sun, title: "Tied to Real Holidays & Seasons", desc: "AI knows when to push spring specials, holiday deals, and slow-season offers." },
  { icon: Rocket, title: "Ready to Execute", desc: "No brainstorming needed. Just grab the copy and run it." },
];
...
export default function PromoPlanner() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data, error: fnError } = await supabase.functions.invoke("create-promo-planner-checkout", {
        body: { ...form },
      });
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { step: "1", title: "Tell Us Your Business", desc: "Industry, location, and what you sell — we handle the rest." },
              { step: "2", title: "Get a Monthly Calendar", desc: "AI builds a promotion plan tied to seasons, holidays, and local events." },
