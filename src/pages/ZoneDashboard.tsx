    ...
    <ZoneThemeWrapper className="min-h-screen pb-24" style={{ background: "#0a0a0a", color: "#e5e5e5" }}>
      ...
      >
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/")} className="h-9 w-9 rounded-lg overflow-hidden flex items-center justify-center transition-all active:scale-90" style={{ background: "#000", boxShadow: "0 0 14px rgba(249,115,22,0.6), 0 0 28px rgba(249,115,22,0.25)" }}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => navigate("/timer")} className="h-8 w-8 rounded-full flex items-center justify-center transition-all active:scale-90" style={{ background: "rgba(0,240,255,0.12)", boxShadow: "0 0 8px rgba(0,240,255,0.25)" }}>
      ...
      <main className="max-w-md sm:max-w-lg md:max-w-2xl lg:max-w-3xl mx-auto px-4 pt-4 pb-4 space-y-4">
        ...
        {/* ── 1. Stats Banner ────────── */}
        <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(0,0,0,0.4)", border: "1px solid rgba(249,115,22,0.15)" }}>
          ...
          <div className="px-4 pb-3 flex items-center gap-2">
            ...
            <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
              <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #f97316, #fb923c)" }} initial={{ width: 0 }} animate={{ width: `${progressPct}%` }} transition={{ duration: 0.8, ease: "easeOut" }} />
            </div>
            {nextLevel && <span className="text-xs font-bold shrink-0" style={{ color: "#525252" }}>{nextLevel.min - ptsTotal} to go</span>}
          </div>
          {currentProgram !== "—" && (
          ...
          <div className="flex" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            <button
              onClick={() => { if (!alreadyCheckedInToday) setShowCheckInChoice(true); }}
              ...
              style={{ color: alreadyCheckedInToday ? "#525252" : "#00f0ff", opacity: alreadyCheckedInToday ? 0.6 : 1 }}
            >
              {alreadyCheckedInToday ? <Check size={14} /> : <MapPin size={14} />}
              <span className="text-xs font-bold uppercase tracking-widest">{alreadyCheckedInToday ? "Checked In ✓" : "Check In"}</span>
            </button>
            <div className="w-px" style={{ background: "rgba(255,255,255,0.06)" }} />
            <button onClick={() => window.dispatchEvent(new Event("open-prove-it-zone"))} className="flex-1 flex items-center justify-center gap-2 py-3 transition-all active:scale-[0.98]" style={{ color: "#f97316" }}>
              <Trophy size={14} />
        ...
        <div className="flex rounded-xl p-1" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
          {TABS.map(({ key, label, icon: Icon }) => (
            <button
              ...
              style={activeTab === key ? { background: "linear-gradient(135deg, #f97316, #ea580c)", color: "#fff", boxShadow: "0 4px 12px rgba(249,115,22,0.3)" } : { color: "#525252" }}
            >
        ...
        {safeLocalStorage.getItem("m2-paused-workout") && (
          <button onClick={() => window.dispatchEvent(new Event("resume-workout-zone"))} className="w-full rounded-xl p-3 flex items-center justify-center gap-2 font-bold text-xs uppercase tracking-widest animate-pulse" style={{ background: "rgba(249,115,22,0.1)", border: "1px solid rgba(249,115,22,0.3)", color: "#f97316" }}>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeTab} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.15 }}>
            {activeTab === "lifts" && <Suspense fallback={<TabLoader />}><div className="space-y-4"><ProgressCharts /><OverloadCard /></div></Suspense>}
        ...
        {showTabTip && <FeatureLearningModal tip={currentTip} onContinue={dismissTip} onDismiss={dismissTip} />}

        {/* ── 5. Smart Start Button (bottom of scroll) ── */}
        <button
          onClick={() => {
            if (hasActiveWorkout) {
          ...
          className="w-full rounded-2xl flex items-center justify-center gap-3 font-black text-base uppercase tracking-wider transition-all active:scale-[0.96]"
          style={{
            height: "54px",
        ...
        >
          {hasActiveWorkout ? (
            <><Play size={20} /> Start Today's Workout</>
          ) : (
            <><Mic size={20} /> Log Your Workout</>
          )}
        </button>
      </main>
      ...
      <AnimatePresence>
        {showCheckInChoice && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)" }} onClick={() => setShowCheckInChoice(false)}>
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="w-full max-w-sm rounded-2xl p-6 space-y-4" style={{ background: "#141414", border: "1px solid rgba(255,255,255,0.08)" }} onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-2">
                <button onClick={() => handleCheckIn("matts_gym")} disabled={checkingIn} className="w-full rounded-xl py-4 text-xs font-black uppercase tracking-widest transition-all active:scale-[0.97]" style={{ background: "linear-gradient(135deg, #f97316, #ea580c)", color: "#fff" }}>🏋️ Matt's Gym</button>
                <button onClick={() => handleCheckIn("on_your_own")} disabled={checkingIn} className="w-full rounded-xl py-4 text-xs font-black uppercase tracking-widest transition-all active:scale-[0.97]" style={{ background: "rgba(0,240,255,0.1)", border: "1px solid rgba(0,240,255,0.3)", color: "#00f0ff" }}>💪 On Your Own</button>
              </div>
              <button onClick={() => setShowCheckInChoice(false)} className="w-full text-center text-xs font-bold uppercase tracking-widest py-2" style={{ color: "#525252" }}>Cancel</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
