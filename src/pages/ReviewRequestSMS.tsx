...
import { CheckCircle, Loader2, MessageSquare, Star, Repeat } from "lucide-react";

const FEATURES = [
  {
    icon: Repeat,
    title: "Automatic After Every Job",
    desc: "The moment you submit a customer's number, the text goes out. No manual sending, ever.",
  },
  {
    icon: MessageSquare,
    title: "Custom Message",
    desc: "Text is personalized with your business name so it feels human, not spammy.",
  },
  {
    icon: Star,
    title: "Google Review Link Included",
    desc: "Customer gets a direct link straight to your Google review page — one tap and they're done.",
  },
];

export default function ReviewRequestSMS() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke("create-review-request-checkout", {
        body: form,
      });
    ...
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <section className="px-4 pt-20 pb-16 text-center max-w-3xl mx-auto">
        ...
        <h1 className="text-4xl sm:text-5xl font-black leading-tight mb-4">
          Turn Every Job Into<br />a 5-Star Review
        </h1>
        <p className="text-slate-400 text-lg leading-relaxed mb-8 max-w-xl mx-auto">
          After each job, your customer automatically gets a text asking for a Google review. No chasing, no asking awkwardly — it just happens.
        </p>
      ...
      <section className="px-4 pb-12 max-w-2xl mx-auto">
        <h2 className="text-center text-xl font-bold text-slate-300 mb-6">Here's how it works</h2>
        <div className="flex flex-col sm:flex-row gap-4 text-center">
          {[
            { step: "1", label: "Finish the job" },
            { step: "2", label: "Submit customer's phone to your portal" },
        ...
        <Card className="bg-slate-800 border-slate-700">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <Label className="text-slate-300 text-sm">Google Review URL <span className="text-slate-500 font-normal">(optional — we'll help you find it)</span></Label>
                <Input
