...
import { CheckCircle, Loader2, ArrowRight, RefreshCw, UserCheck, Send, TrendingUp } from "lucide-react";
...
const BENEFITS = [
  { icon: RefreshCw, title: "Win Back Lapsed Customers", desc: "Re-engage customers who haven't bought in 60, 90, or 120+ days." },
  { icon: UserCheck, title: "Personalized AI Messages", desc: "Each email is tailored to the customer's history and your business type." },
  { icon: Send, title: "Monthly Automated Sends", desc: "AI identifies lapsed customers and sends reactivation emails every month." },
  { icon: TrendingUp, title: "Proven ROI", desc: "Reactivating just 2-3 old customers per month pays for the entire service." },
];

export default function ReactivationEmails() {
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center px-6">
        <div className="text-center max-w-sm">
          ...
          <p className="text-slate-400 leading-relaxed">Your 7-day free trial is active. Matt will reach out to connect your customer list.</p>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data, error: fnError } = await supabase.functions.invoke("create-reactivation-email-checkout", {
        body: { ...form },
      });
    ...
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <div className="bg-slate-900 px-6 py-16 text-center border-b border-slate-800">
        <span className="inline-block text-[11px] font-bold uppercase tracking-widest text-orange-500 bg-orange-500/10 px-3 py-1 rounded-full mb-4">
          AI Customer Reactivation Email
        </span>
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { step: "1", title: "Connect Your Customer List", desc: "Upload or sync your customer data — we identify who's gone cold." },
              { step: "2", title: "AI Writes the Emails", desc: "Personalized win-back messages for each lapsed customer segment." },
              { step: "3", title: "Customers Come Back", desc: "Automated monthly sends bring old customers back through the door." },
            ].map((s) => (
