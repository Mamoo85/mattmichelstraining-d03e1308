...
import { CheckCircle, Loader2, ArrowRight, Scale, BookOpen, Phone, Search, Smartphone, Star } from "lucide-react";
...
const PLANS = [
  {
    ...
    setup: "$1,499",
    monthly: "$99/mo",
    description: "Professional site for solo attorneys and small firms.",
    includes: [
  ...
  },
  {
    ...
    setup: "$2,499",
    monthly: "$149/mo",
    description: "Multi-attorney firms needing full credibility and intake.",
    includes: [
      "Everything in Standard",
      "Multi-attorney bio pages",
      "Case results / verdicts section",
      "Client testimonials",
      "Blog/articles section (SEO)",
...
const FEATURES = [
  { icon: Scale, label: "Practice area pages", sub: "Dedicated pages for each area of law you practice" },
  { icon: Phone, label: "Free consult form", sub: "Capture leads 24/7 with a simple intake form" },
  { icon: BookOpen, label: "Case results", sub: "Showcase wins and verdicts to build credibility" },
  { icon: Search, label: "Local SEO", sub: "Rank for 'attorney near me' and practice-area searches" },
  { icon: Star, label: "Testimonials", sub: "Feature client reviews front and center" },
  { icon: Smartphone, label: "Mobile-first", sub: "Clients searching on their phone see a fast, clean site" },
];

const FAQS = [
  { q: "What makes a law firm website different from a generic business site?", a: "Potential clients are evaluating trust before they ever call. They need to see your experience, case results, and practice areas clearly — and have an easy way to reach you. We build for that decision-making process." },
  { q: "Do I need to provide all the content?", a: "We handle copywriting and SEO structure. You provide attorney photos, practice area details, and any case results or testimonials you want featured." },
  { q: "How long does the build take?", a: "Standard sites are live within 10–14 days. Pro sites with multiple attorneys and blog sections take 2–3 weeks." },
  { q: "Will this help me rank on Google?", a: "Yes. We set up local SEO schema, Google Business Profile optimization, and practice-area keyword pages so clients searching for your type of law in your city find you first." },
];

export default function LegalWebDesign() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.email || !form.business_name) { toast.error("Firm name and email are required"); return; }
    setSubmitting(true);
    try {
        ...
        <div className="max-w-3xl mx-auto px-6 py-12">
          ...
          <div className="bg-card border border-border p-6 mb-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground block mb-1">Firm Name *</label>
                  <input value={form.business_name} onChange={e => setForm(f => ({...f, business_name: e.target.value}))} required placeholder="Smith & Associates Law"
                ...
                <div>
                  ...
                  <input value={form.contact_name} onChange={e => setForm(f => ({...f, contact_name: e.target.value}))} placeholder="John Smith, Esq."
                    className="w-full bg-background border border-border px-3 py-2.5 text-sm focus:ring-1 focus:ring-primary outline-none" />
                </div>
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground block mb-1">Email *</label>
                  <input type="email" required value={form.email} onChange={e => setForm(f => ({...f, email: e.target.value}))} placeholder="you@lawfirm.com"
                    className="w-full bg-background border border-border px-3 py-2.5 text-sm focus:ring-1 focus:ring-primary outline-none" />
                </div>
                <div>
          ...
          <div className="bg-card border border-border p-5 mb-10 flex items-start gap-4">
            ...
            <p className="text-[13px] text-muted-foreground leading-relaxed">
              <span className="font-bold text-foreground">I'm Matt Michels — Grosse Pointe, MI.</span> I build websites for local professionals. No agency fees, no locked contracts — just a site that brings in new clients and a real person you can text when you need a change.
            </p>
          </div>
