...
export default function AIInventoryAlerts() {
  ...
  const features = [
    { icon: AlertTriangle, title: "Low Stock Alerts", desc: "Instant notifications when items drop below your par levels" },
    { icon: TrendingDown, title: "Usage Tracking", desc: "AI estimates days until stockout based on consumption patterns" },
    { icon: ShoppingCart, title: "Reorder Suggestions", desc: "Recommended quantities and supplier suggestions for each item" },
    { icon: Package, title: "Save 20-30%", desc: "Avoid emergency ordering markups by reordering before you run out" },
  ];
      ...
      <div className="max-w-5xl mx-auto px-4 py-16">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-primary/10 text-primary px-4 py-1.5 rounded-full text-sm font-bold mb-4">
            <Package size={16} /> Inventory Automation
          </div>
          <h1 className="text-4xl md:text-5xl font-black mb-4">AI Inventory Reorder Alerts</h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Never run out of critical supplies again. Set par levels, and our AI monitors stock, predicts stockouts, and sends reorder alerts with supplier suggestions.
          </p>
        ...
        <Card className="max-w-md mx-auto border-primary/30">
          <CardContent className="p-6 space-y-4">
            ...
            <Input placeholder="Industry (e.g. Restaurant, Retail, Auto Shop)" value={form.industry} onChange={e => setForm(p => ({ ...p, industry: e.target.value }))} />
