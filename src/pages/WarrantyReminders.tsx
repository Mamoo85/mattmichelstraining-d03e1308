...
import { CheckCircle, Loader2, ArrowRight, Bell, Wrench, ShieldCheck, Users } from "lucide-react";
...
const BENEFITS = [
  { icon: Bell, title: "Auto-Remind Before Expiry", desc: "Customers get a text before their warranty runs out — perfectly timed." },
  { icon: Wrench, title: "Book More Service Calls", desc: "Reminders turn into appointments. More service calls, more revenue." },
  { icon: ShieldCheck, title: "Reduce Warranty Disputes", desc: "When customers know their status, disputes drop dramatically." },
  { icon: Users, title: "Customer Retention", desc: "Stay top-of-mind. Customers who hear from you stick with you." },
];

export default function WarrantyReminders() {
  const [form, setForm] = useState({ name: "", businessName: "", email: "", phone: "", industry: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data, error: fnError } = await supabase.functions.invoke("create-warranty-reminder-checkout", {
        body: { ...form },
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <div className="bg-[#0a0a0f] px-6 py-16 text-center border-b border-slate-800">
        ...
        <h1 className="text-3xl md:text-4xl font-black mb-4 leading-tight">
          Never let a warranty expire without a follow-up.
        </h1>
        <p className="text-2xl font-black text-cyan-500 mb-2">$29<span className="text-sm font-normal text-slate-400">/mo</span></p>
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { step: "1", title: "Upload Warranty Data", desc: "Customer names, install dates, and warranty lengths — we do the rest." },
              { step: "2", title: "AI Schedules Reminders", desc: "Texts go out 30, 14, and 7 days before each warranty expires." },
              { step: "3", title: "Book the Service Call", desc: "Customers call you to renew or schedule maintenance. Revenue flows." },
            ].map((s) => (
        ...
        <Card className="bg-slate-800/50 border-slate-700 p-6 mb-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={loading} className="w-full bg-cyan-500 hover:bg-cyan-600 text-white py-3 font-bold text-sm uppercase tracking-widest">
              ...
              {loading ? "Redirecting to checkout..." : "Start Free Trial — $29/mo"}
            </Button>
