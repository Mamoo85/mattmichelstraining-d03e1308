...
import { Trash2, ChevronDown, ChevronUp, Plus, Minus, Link, MessageSquare } from "lucide-react";
...
const ExerciseCard = ({ exercise, index, onUpdate, onRemove }: ExerciseCardProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Flag for coach */}
      <div className={cn("flex items-center justify-between px-3 py-3 border-t border-border", exercise.flagForCoach && "bg-primary/10")}>
        <label htmlFor={`flag-${index}`} className="text-xs font-bold uppercase tracking-widest text-foreground cursor-pointer select-none">
          🏴 Flag for Coach Matt
        </label>
        <Switch
          id={`flag-${index}`}
          checked={exercise.flagForCoach}
          onCheckedChange={(checked) => onUpdate({ flagForCoach: checked })}
        />
      </div>
    </div>
  );
