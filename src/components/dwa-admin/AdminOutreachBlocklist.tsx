    ...
    <div className="space-y-6 text-white">
      ...
      <div className="flex gap-2 items-center">
        ...
        <button
          ...
          onClick={() => load()}
          disabled={loading}
          className="px-4 py-2 bg-[#00d4ff]/20 border border-[#00d4ff]/40 rounded-md text-sm text-[#00d4ff] hover:bg-[#00d4ff]/30 disabled:opacity-50 disabled:cursor-wait"
        >
      ...
      {error && (
        <div className="bg-rose-500/10 border border-rose-500/40 rounded-md px-3 py-2 text-sm text-rose-300">
          ⚠️ {error}
        </div>
      )}
      {lastLoaded && !error && (
        <div className="text-xs text-white/40">Last loaded: {lastLoaded.toLocaleTimeString()}</div>
      )}
