...
interface GiftProtocolModalProps {
  protocolId: string;
...
const GiftProtocolModal = ({ protocolId, protocolTitle, onClose, onGifted }: GiftProtocolModalProps) => {
  ...
  const { data: users = [] } = useQuery({
    queryKey: ["admin-users-for-gift", search],
    queryFn: async () => {
    ...
    try {
      ...
      const { data: newProto, error: protoErr } = await supabase
        ...
        .insert({
          ...
          is_template: false,
          is_default: false,
          gifted_to: selectedUser.id,
          gift_message: giftMessage || null,
          source_template_id: protocolId,
        })
        .select("id")
        .single();
      if (protoErr || !newProto) throw protoErr || new Error("Failed to create protocol");

      // Clone exercises
      if (templateExercises && templateExercises.length > 0) {
        const rows = templateExercises.map((ex) => ({
        ...
        const { error: insertErr } = await supabase.from("protocol_exercises").insert(rows);
      ...
      await supabase.from("notifications").insert({
        user_id: selectedUser.id,
        type: "protocol_gift",
        title: `New Protocol from Coach Matt`,
        body: giftMessage || `You've been gifted "${protocolTitle}" — check your Fix It section.`,
        link: "/dashboard",
      });

      toast({ title: `Protocol gifted to ${selectedUser.name}` });
      onGifted();
    } catch (err: any) {
      ...
      <div className="bg-card border border-border w-full max-w-md p-5 space-y-4 shadow-xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold uppercase tracking-widest text-foreground">Gift Protocol</h3>
          </div>
        ...
        <p className="text-xs text-muted-foreground">
          Gifting "<span className="text-foreground font-bold">{protocolTitle}</span>" — select a user below.
        </p>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Hey! I put this together for you based on what we discussed..."
...
export default GiftProtocolModal;
