...
const LARA_LICENSE_GROUPS = [
  {name:"cosmetology",codes:"1101,1102,1103,1104,1105,1106,1107,1108,1109,1110,1111,1112,1113,1114,1115,1116,1117,1118,1119,1120",industry:"hair salon",mapsQuery:"hair salon OR beauty salon OR nail salon OR barbershop"},
  {name:"dental",codes:"3001,3002,3003,3004,3005,3006",industry:"dental",mapsQuery:"dental office OR dentist OR dental clinic"},
  {name:"physical_therapy",codes:"6001,6002,6003,6101,6102,6103",industry:"physical therapy",mapsQuery:"physical therapy clinic OR PT clinic OR rehabilitation clinic"},
  {name:"veterinary",codes:"7001,7002,7003",industry:"veterinary",mapsQuery:"veterinary clinic OR animal hospital OR veterinarian"},
];
...
function scoreRow(r:ProspectRow):number { let s=2; if(r.phone)s+=2; if(r.website)s+=2; if(r.email)s+=3; return Math.min(s,9); }
...
async function scanLARANewLicenses(group:typeof LARA_LICENSE_GROUPS[number]):Promise<{county:string;count:number}[]> {
  ...
  try {
    const res=await fetch(`https://cofs.lara.state.mi.us/SearchApi/Search/Search?entityType=IND&searchType=DATE&dateSearchType=LICENSURE&dateFrom=${twoWeeksAgo}&licenseTypes=${group.codes}`,{headers:{"User-Agent":"DWA-MissedCallScanner/1.0 matt@detroitwebagent.com"},signal:AbortSignal.timeout(15_000)});
    if(!res.ok)return [];
    const data=await res.json();
    const licensees=data?.Result?.Items||data?.Items||[];
    const countByCounty:Record<string,number>={};
    for(const lic of licensees){const county=(lic.County||lic.county||"").trim();if(county)countByCounty[county]=(countByCounty[county]||0)+1;}
    return Object.entries(countByCounty).map(([county,count])=>({county,count})).sort((a,b)=>b.count-a.count).slice(0,3);
  }catch(e){console.warn(`[mc-scanner] LARA ${group.name}:`,e instanceof Error?e.message:e);return [];}
}
...
async function googleMapsSearch(query:string,cityState:string):Promise<ProspectRow[]> {
  ...
  try {
    const res=await fetch(`https://maps.googleapis.com/maps/api/place/textsearch/json?query=${encodeURIComponent(`${query} ${cityState}`)}&key=${GOOGLE_MAPS_API_KEY}`,{signal:AbortSignal.timeout(10_000)});
    if(!res.ok)return [];
    const data=await res.json();
    ...
    for(const place of (data.results||[]).slice(0,15)){
      ...
      if(place.place_id){try{const dr=await fetch(`https://maps.googleapis.com/maps/api/place/details/json?place_id=${place.place_id}&fields=formatted_phone_number,website&key=${GOOGLE_MAPS_API_KEY}`,{signal:AbortSignal.timeout(7_000)});if(dr.ok){const dd=await dr.json();phone=dd.result?.formatted_phone_number||null;website=dd.result?.website?.replace(/\/$/,"")||null;}}catch(_){}}
      results.push({business_name:String(place.name).trim(),city,industry:null,phone:phone?.replace(/\D/g,"").slice(-10)||null,website,email:null,source:"google_maps_mc",score:0,drip_campaign_status:{current_stage:"0_New_Extracted_Lead"}});
    }
  }catch(e){console.warn(`[mc-scanner] maps ${query} ${cityState}:`,e instanceof Error?e.message:e);}
  return results;
}
...
serve(wrapServe("missed-call-prospect-scanner",async(_req)=>{
  ...
  for(const prospect of allProspects){
    ...
    const {error}=await sb.from("outreach_leads").insert({business_name:prospect.business_name,city:prospect.city,industry:prospect.industry,phone:prospect.phone,website:prospect.website,email:prospect.email,source:prospect.source,score:scoreRow(prospect),drip_campaign_status:{current_stage:"0_New_Extracted_Lead"}});
    if(!error){inserted++;if(emailKey)existingKeys.add(emailKey);existingKeys.add(bizKey);}
  }
  await sb.from("agent_heartbeats").upsert({agent_name:"missed-call-prospect-scanner",last_beat:new Date().toISOString(),status:"ok",metadata:{inserted,skipped,total_scanned:allProspects.length,lara_signals:laraSignalSummary,duration_ms:Date.now()-startedAt}},{onConflict:"agent_name"});
  if(laraSignalSummary.length>0&&TWILIO_PHONE)await sendSMS(ADMIN_PHONE,TWILIO_PHONE,`[MC Scanner] ${inserted} new Missed-Call prospects queued. LARA signals: ${laraSignalSummary.join(" | ")}`,"missed_call").catch(()=>{});
  return new Response(JSON.stringify({ok:true,inserted,skipped,total_scanned:allProspects.length,lara_signals:laraSignalSummary,duration_ms:Date.now()-startedAt}),{headers:{...corsHeaders,"Content-Type":"application/json"}});
}));
