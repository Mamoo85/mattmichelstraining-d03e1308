...
serve(async (req) => {
  ...
  try {
    ...
    await sb.from("landlord_letter_clients").upsert(
      { email, contact_name: name || null, phone: phone || null, state: body.state || 'MI', property_count: parseInt(body.propertyCount) || 1, active: false },
      { onConflict: "email" }
    );
