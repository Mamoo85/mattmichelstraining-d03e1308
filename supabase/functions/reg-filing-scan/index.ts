...
async function processClient(client: any): Promise<{ items: number; criticals: number }> {
  ...
  if (criticalCount > 0 && client.phone) {
    await sendSMS(client.phone, TWILIO_PHONE, `${criticalCount} critical regulatory finding${criticalCount > 1 ? "s" : ""} need your attention. Details in your email. — Matt (313) 806-4952`, "reg_filing_monitor");
  }
...
async function deadlineCheck(): Promise<string> {
  ...
  for (const dl of deadlines || []) {
    ...
    if (daysUntil <= 3 && client?.phone) {
      await sendSMS(client.phone, TWILIO_PHONE, `Heads up: "${dl.title}" is due in ${daysUntil} day${daysUntil !== 1 ? "s" : ""}. Check your email. — Matt (313) 806-4952`, "reg_filing_monitor");
    }
