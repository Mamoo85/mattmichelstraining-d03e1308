  ...
  try {
    ...
    for (const opp of opps) {
      ...
      const row = {
        ...
        vertical: naicsToVertical(opp.naicsCode),
        value_usd: award.amount ? parseFloat(award.amount) : null,
        predicted_needs: opp.title ? [opp.title.slice(0, 200)] : [],
        recommended_pitch: (() => { const vert = naicsToVertical(opp.naicsCode); const wage = PREVAILING_WAGE_BY_VERTICAL[vert] || 36; return `MI gov contract posted ${opp.postedDate || ""}: ${opp.title || ""}. NAICS ${opp.naicsCode || "n/a"}. Response deadline ${opp.responseDeadLine || "TBD"}. Est. prevailing wage ${wage}/hr (Metro Detroit, ${vert.replace(/_/g," ")}).`; })(),
