...
serve(async (_req) => {
  ...
  try {
    ...
    for (const client of clients) {
      try {
        ...
        const content = aiData?.content?.[0]?.text || "<p>KPI report unavailable this week.</p>";
        if (RESEND_API_KEY) {
  ...
  } catch (e: any) { console.error("[KPI-EMAIL] Fatal:", e); return new Response(JSON.stringify({ error: e.message }), { status: 500 }); }
});
