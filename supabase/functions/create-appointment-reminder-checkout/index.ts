...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: "usd", recurring: { interval: "month" }, unit_amount: 3900, product_data: { name: "Appointment Reminder SMS — $39/month", description: "Automated SMS reminders 24hr and 1hr before every appointment" } }, quantity: 1 }],
