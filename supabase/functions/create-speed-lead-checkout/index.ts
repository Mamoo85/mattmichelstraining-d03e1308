    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          ...
          unit_amount: 3900,
          product_data: { name: "Speed-to-Lead", description: "Auto-text new leads within 60 seconds — beat competitors to the response." },
        },
        quantity: 1,
      }],
      metadata: { type: "speed_lead_subscription", email, name: name || "", businessName, phone: phone || "" },
      success_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/speed-to-lead?status=success`,
      cancel_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/speed-to-lead`,
    });
