...
serve(async (req) => {
  ...
  try {
    ...
    const { email, name, phone, company_name } = body;
    ...
    await sb.from("commercial_lease_clients").upsert(
      { email, contact_name: name || null, phone: phone || null, company_name: company_name || null, active: false },
      { onConflict: "email" }
    );
