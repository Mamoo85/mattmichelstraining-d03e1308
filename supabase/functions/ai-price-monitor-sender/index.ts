      ...
      try {
        ...
        if (RESEND_API_KEY) {
          await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              from: "M² Price Monitor <matt@notify.m2training.com>",
              to: [client.email],
              subject: `Weekly Price Intel — ${client.business_name}`,
              html: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:20px;background:#1e293b;color:#e2e8f0;border-radius:12px;"><h2 style="color:#e8621a;">💰 Competitor Price Monitor</h2>${report}<hr style="border-color:#334155;"><p style="color:#64748b;font-size:12px;">Powered by M² Performance</p></div>`,
            }),
          });
        }
