...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: "usd", recurring: { interval: "month" }, unit_amount: 2900, product_data: { name: "AI Social Caption Pack — $29/month", description: "30 AI-written social media captions per month. Copy, paste, post." } }, quantity: 1 }],
      metadata: { type: "caption_pack_subscription", email, name: name || "", businessName, industry: industry || "", platforms: platforms || "Facebook, Instagram" },
      success_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/ai-caption-pack?status=success`,
      cancel_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/ai-caption-pack`,
    });
