...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: "usd", recurring: { interval: "month" }, unit_amount: 3900, product_data: { name: "Review Request SMS — $39/month", description: "Auto-text customers after each job asking for a Google review." } }, quantity: 1 }],
