...
serve(async (req) => {
  ...
  try {
    ...
    await sb.from("menu_engineering_clients").upsert(
      { email, contact_name: name || null, phone: phone || null, restaurant_name: body.restaurantName || null, cuisine_type: body.cuisineType || null, active: false },
      { onConflict: "email" }
    );
