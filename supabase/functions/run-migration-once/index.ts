// One-time migration runner + test SMS — DELETE THIS FUNCTION after use.
// Applies mortgage_radar geo columns to the live DB and sends test texts.
import { serve } from "https://deno.land/std@0.190.0/http/server.ts";
import { createClient } from "npm:@supabase/supabase-js@2.57.2";
import { sendSMS } from "../_shared/twilio.ts";

const SUPABASE_URL = Deno.env.get("SUPABASE_URL")!;
const SERVICE_ROLE = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
const SECRET = "mig-2026-05-02";

// SE Michigan zips — Wayne / Oakland / Macomb counties + suburbs
const SE_MICHIGAN_ZIPS = [
  // Wayne County — Detroit core
  "48201","48202","48204","48205","48206","48207","48208","48209","48210","48211",
  "48213","48214","48215","48216","48217","48219","48221","48223","48224","48225",
  "48226","48227","48228","48229","48234","48235","48238","48239","48240","48243",
  // Wayne County — suburbs
  "48120","48121","48122","48124","48125","48126","48127","48128","48134","48141",
  "48150","48151","48152","48153","48154","48185","48186",
  "48183","48184","48192","48193","48195","48101",
  // Macomb County — Warren / Sterling Heights / Roseville / St. Clair Shores
  "48088","48089","48091","48092","48093","48310","48311","48312","48313","48314",
  "48315","48316","48317","48318","48066","48080","48081","48082","48083",
  // Oakland County — Troy / Royal Oak / Southfield / Pontiac / Auburn Hills
  "48009","48067","48068","48069","48070","48071","48072","48073","48075","48076",
  "48220","48237","48301","48302","48303","48304","48306","48307","48308","48309",
  "48320","48321","48322","48323","48324","48325","48327","48328","48329","48330",
  "48331","48334","48335","48336","48340","48341","48342","48346","48350","48360",
  "48362","48363","48374","48375","48376","48377",
];

serve(async (req) => {
  const url = new URL(req.url);
  if (url.searchParams.get("secret") !== SECRET) {
    return new Response(JSON.stringify({ error: "forbidden" }), { status: 403 });
  }

  const sb = createClient(SUPABASE_URL, SERVICE_ROLE, { auth: { persistSession: false } });
  const results: Record<string, unknown> = {};

  // 1. Read current state
  const { data: before } = await sb
    .from("mortgage_radar_clients")
    .select("id, email, zip_codes, coverage_counties, coverage_regions")
    .eq("email", "matt@detroitwebagent.com")
    .single();
  results.before = before ? {
    zip_count: before.zip_codes?.length ?? 0,
    coverage_counties: before.coverage_counties,
    coverage_regions: before.coverage_regions,
  } : null;

  // 2. Try to set coverage_regions (works if column exists after Lovable migration)
  const { error: regErr } = await (sb.from as any)("mortgage_radar_clients")
    .update({ coverage_regions: ["Southeast Michigan"], coverage_counties: [] as string[] })
    .eq("email", "matt@detroitwebagent.com");
  results.coverage_regions_update = regErr ? `err: ${regErr.message}` : "ok";

  // 3. Always set broad SE Michigan zip codes as fallback for digest matching
  const { error: zipErr } = await sb
    .from("mortgage_radar_clients")
    .update({ zip_codes: SE_MICHIGAN_ZIPS })
    .eq("email", "matt@detroitwebagent.com");
  results.zip_codes_update = zipErr ? `err: ${zipErr.message}` : `set ${SE_MICHIGAN_ZIPS.length} zips`;

  // 4. Read final state
  const { data: after } = await sb
    .from("mortgage_radar_clients")
    .select("id, email, zip_codes, coverage_counties, coverage_regions")
    .eq("email", "matt@detroitwebagent.com")
    .single();
  results.after = after ? {
    zip_count: after.zip_codes?.length ?? 0,
    coverage_counties: after.coverage_counties,
    coverage_regions: after.coverage_regions,
  } : null;

  // 5. Count leads available for digest
  const { count: leadCount } = await sb
    .from("mortgage_radar_leads")
    .select("id", { count: "exact", head: true })
    .gte("created_at", new Date(Date.now() - 24 * 3600_000).toISOString());
  results.leads_last_24h = leadCount ?? 0;

  // 6. Send test SMS to Matt and his brother
  const testMsg = `🏠 Mortgage Radar test text — system is live. ${leadCount ?? 0} new lead${leadCount !== 1 ? "s" : ""} scanned in SE Michigan today. — DWA`;
  const [matt, brother] = await Promise.allSettled([
    sendSMS(sb, "+13139921219", testMsg),
    sendSMS(sb, "+13136719441", testMsg),
  ]);
  results.sms_matt = matt.status === "fulfilled" ? "sent" : (matt as PromiseRejectedResult).reason?.message;
  results.sms_brother = brother.status === "fulfilled" ? "sent" : (brother as PromiseRejectedResult).reason?.message;

  return new Response(JSON.stringify({ ok: true, ...results }, null, 2), {
    headers: { "Content-Type": "application/json" },
  });
});
