...
serve(async (req) => {
  ...
  try {
    ...
    await sb.from("promo_blaster_clients").upsert({ email, business_name: businessName, contact_name: name || null, phone: phone || null, business_type: businessType || null, city: city || null, state: state || "MI", active: false }, { onConflict: "email" });
