...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: "usd", recurring: { interval: "month" }, unit_amount: 4900, product_data: { name: "Quote Follow-Up SMS — $49/month", description: "Automated 3-text follow-up sequence after every quote. Closes more jobs on autopilot." } }, quantity: 1 }],
      metadata: { type: "quote_followup_subscription", email, name: name || "", businessName, phone: phone || "" },
      success_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/quote-followup-sms?status=success`,
      cancel_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/quote-followup-sms`,
    });
