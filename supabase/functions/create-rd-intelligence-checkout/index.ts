...
serve(async (req) => {
  ...
  try {
    ...
    const { email, name, phone, company_name, research_topics } = body;
    ...
    await sb.from("rd_intelligence_clients").upsert(
      { email, contact_name: name || null, phone: phone || null, company_name: company_name || null, research_topics: research_topics || null, active: false },
      { onConflict: "email" }
    );
