...
serve(wrapServe("techalert-followup-drip", async (req) => {
  ...
  // Per-client / test-mode params
  let clientId: string | null = null;
  let overrideTo: string | null = null;
  let forceTouch: Touch | null = null;
  try {
    const b = await req.clone().json();
    clientId = b?.client_id ?? null;
    overrideTo = b?.override_to ?? null;
    if (b?.force_touch && ["d3", "d7", "d14"].includes(b.force_touch)) forceTouch = b.force_touch as Touch;
  } catch { /* defaults */ }
  ...
  try {
    ...
    for (const t of targets) {
      ...
      // Determine which touch to send (force_touch overrides timing logic for E2E tests)
      let touch: Touch | null = forceTouch ?? null;
      if (!touch) {
      ...
      // Blocklist gate: skip unless we're in override/test mode
      if (!overrideTo) {
      ...
      const result = await sendFollowup(sb, emailTo, t.owner_name, t.company_name, t.role, touch);
