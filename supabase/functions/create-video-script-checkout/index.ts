...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: "usd", recurring: { interval: "month" }, unit_amount: 3900, product_data: { name: "AI Video Script Writer — $39/month", description: "8 short-form video scripts per month for TikTok and Reels" } }, quantity: 1 }],
