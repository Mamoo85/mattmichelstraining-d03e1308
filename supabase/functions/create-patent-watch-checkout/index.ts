...
serve(async (req) => {
  ...
  try {
    ...
    const { email, name, phone, company_name, industry } = body;
    ...
    await sb.from("patent_watch_clients").upsert(
      { email, contact_name: name || null, phone: phone || null, company_name: company_name || null, industry: industry || null, active: false },
      { onConflict: "email" }
    );
