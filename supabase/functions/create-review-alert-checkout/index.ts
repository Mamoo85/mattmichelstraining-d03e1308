...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: "usd", recurring: { interval: "month" }, unit_amount: 1900, product_data: { name: "Review Alert SMS — $19/month", description: "Instant SMS notification when a new review is posted on Google." } }, quantity: 1 }],
