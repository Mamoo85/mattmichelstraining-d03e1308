...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: "usd", recurring: { interval: "month" }, unit_amount: 4900, product_data: { name: "AI Proposal Generator — $49/month", description: "Unlimited AI-written professional proposals. Submit project details, get a polished proposal back in minutes." } }, quantity: 1 }],
      metadata: { type: "proposal_generator_subscription", email, name: name || "", businessName, industry: industry || "" },
      success_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/ai-proposal?status=success`,
      cancel_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/ai-proposal`,
    });
