...
serve(async (req) => {
  ...
  try {
    ...
    const { transaction_id, charge_id } = await req.json();
    if (!charge_id) throw new Error("Missing charge_id");
    ...
    if (transaction_id) {
      await sb.from("transactions").update({ status: "refunded" }).eq("id", transaction_id);
    }
