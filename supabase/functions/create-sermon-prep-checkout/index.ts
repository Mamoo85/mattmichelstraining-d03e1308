...
serve(async (req) => {
  ...
  try {
    ...
    await sb.from("sermon_prep_clients").upsert(
      { email, contact_name: name || null, phone: phone || null, church_name: body.churchName || null, denomination: body.denomination || null, active: false },
      { onConflict: "email" }
    );
