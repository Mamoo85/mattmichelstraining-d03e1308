...
Circling back on my note from a few days ago about ${companyName}.

We help ${trade} businesses capture missed calls, reactivate dead quotes, and surface live homeowner signals before anyone else in their area sees them. Most owners pick up 1–2 extra jobs a month without adding staff.

7-day trial, no credit card. Worth a look?
...
Following up one more time — I know how busy ${trade} season gets.

What the 7-day trial includes (no card, cancel anytime):
• Missed-Call Catch — auto-texts every missed caller within 30 seconds so ${companyName} is always first to respond
• Dead Lead Reactivation — re-engages old quotes that went quiet (you only pay per reply)
• Live homeowner signals in your service area — permits, storm damage, FSBOs, estate sales

If timing is right, reply YES and I'll set it up for ${companyName} today. Takes 10 minutes on my end.
...
async function sendFollowup(
  ...
  const subjects: Record<Touch, string> = {
    d3: `${companyName} — quick follow-up`,
    d7: `${firstName} — 7-day trial, no card`,
    d14: `Last note, ${firstName}`,
  };
