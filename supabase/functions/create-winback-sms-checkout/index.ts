...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: "usd", recurring: { interval: "month" }, unit_amount: 4900, product_data: { name: "Customer Win-Back SMS — $49/month", description: "Monthly AI-written SMS to your lapsed customers. Bring them back automatically." } }, quantity: 1 }],
      metadata: { type: "winback_sms_subscription", email, name: name || "", businessName, phone: phone || "", industry: industry || "" },
      success_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/winback-sms?status=success`,
      cancel_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/winback-sms`,
    });
