...
<p style="margin:0;color:#64748b;font-size:13px">The first newsletter goes out next cycle. Upload your contacts today so nobody gets left out.</p>`,
      ...
      // ── FIELD CRM SUBSCRIPTION ────────────────────────────────────────────
      if (meta.type === "field_crm_subscription" && customerEmail) {
        try {
          const crmSb = createClient(SUPABASE_URL, SUPABASE_SERVICE_KEY);

          // Create client row — visitor_script_key is auto-generated by DB default
          const { data: newClient } = await crmSb.from("field_crm_clients" as any).insert({
            business_name: meta.business_name || customerEmail,
            owner_name: meta.name || null,
            phone: meta.phone || null,
            email: customerEmail,
            website: meta.website || null,
            industry: meta.industry || "hvac",
            plan: meta.plan || "standard",
            monthly_price: meta.plan === "pro" ? 29900 : 19900,
            stripe_customer_id: session.customer as string || null,
            stripe_subscription_id: session.subscription as string || null,
            status: "active",
          }).select("id, visitor_script_key, business_name").single();

          const scriptKey = newClient?.visitor_script_key || "";
...
<p style="margin:0;color:#64748b;font-size:13px">Stuck on the install? Reply to this email or text me at (313) 992-1219 — I'll walk you through it in 5 minutes.</p>`,
          ...
          // Notify Matt
          await notifyMatt(
            `💰 New Field CRM Client — ${meta.business_name || customerEmail} (${meta.plan || "standard"})`,
...
<div style="max-width:560px;margin:0 auto;padding:32px 16px;">
  <div style="background:#0d1f3c;border:1px solid #1e3a5f;border-radius:12px;overflow:hidden;">
    ...
    <div style="padding:28px 32px;color:#e2e8f0;font-size:15px;line-height:1.8;">
      ...
      ${meta.contractor_id ? `<div style="background:#0a2040;border:1px solid #00d4ff30;border-radius:8px;padding:16px;margin:0 0 20px;"><p style="margin:0 0 8px;color:#94a3b8;font-size:13px;">Track your activation status (welcome SMS delivery, payment confirmation):</p><a href="https://detroitwebagent.com/contractor-onboarding-status?contractor_id=${meta.contractor_id}" style="color:#00d4ff;font-weight:700;font-size:14px;word-break:break-all;">detroitwebagent.com/contractor-onboarding-status</a></div>` : ""}
