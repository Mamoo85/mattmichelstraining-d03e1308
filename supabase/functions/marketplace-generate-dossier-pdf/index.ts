...
serve(async (req) => {
  ...
  try {
    ...
    const { data: cached } = await sb.from("marketplace_lead_pdfs")
      ...
      .eq("lead_id", lead_id).eq("buyer_email", email)
      .maybeSingle();
    ...
    const renderUrl = `https://detroitwebagent.com/lead/${lead_id}?print=1&buyer=${encodeURIComponent(email)}`;
    ...
    const path = `${product}/${lead_id}/${email.replace(/[^a-z0-9]/gi, "_")}_${Date.now()}.pdf`;
