...
export async function scanSignals(state = "MI", zipFilter?: string[]): Promise<RawSignal[]> {
  ...
  // BSEED Presale Inspections — FAIL results (gutter/drainage issues cited)
  try {
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          address: addr, city: "Detroit", zip,
          signal_type: "roof_permit_upsell",
          signal_detail: `Detroit presale inspection FAILED: ${addr} — improper drainage and gutter damage cause basement water intrusion that frequently fails presale inspection`,
          ...
          suggested_opener: `Your property at ${addr} failed its presale inspection — poor drainage and missing gutters often contribute to water intrusion that shows up on inspection. We can replace or repair gutters fast and help you pass re-inspection.`,
  ...
  } catch (e) { console.error("[gutters] presale inspections:", e); }
