...
export async function scanSignals(state = "MI", zipFilter?: string[]): Promise<RawSignal[]> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          address: addr, city: "Detroit", zip,
          signal_type: "cofc_mold_water_inspection",
          signal_detail: `CofC expires in ${daysLeft} days — BSEED inspectors look for water staining, mold evidence, and moisture intrusion at renewal. ${addr} latent water damage must be remediated`,
          signal_date: today,
          score,
          source_method: "bseed_cofc_expiring",
          suggested_opener: `Your rental at ${addr} has a Certificate of Compliance expiring in ${daysLeft} days — inspectors commonly fail properties for water staining on ceilings, mold in bathrooms, and wet basements. A pre-inspection moisture assessment protects your rental license.`,
    ...
    if (res.ok) {
      ...
      for (const item of (d?.results ?? [])) {
        ...
        signals.push({
          ...
          score: 7,
          signal_detail: `Foreclosure filing: ${addr} — property entering legal distress. Foreclosed homes commonly need water/mold remediation after vacancy; insurance-grade documentation speeds legal proceedings`,
          source_method: "courtlistener_foreclosure",
          suggested_opener: "Foreclosed homes commonly need water/mold remediation after vacancy — we provide insurance-grade documentation for legal proceedings and can be on-site within 48 hours.",
  ...
  } catch (e) { console.error("[restoration] CourtListener foreclosure:", e); }
