...
export async function scanSignals(state = "MI", zipFilter?: string[]): Promise<RawSignal[]> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          address: addr, city: "Detroit", zip,
          signal_type: "cofc_tree_inspection",
          signal_detail: `CofC expires in ${daysLeft} days — BSEED inspects for overhanging dead limbs and hazardous trees as exterior safety conditions at renewal. ${addr} landlord needs yard cleared`,
          signal_date: today,
          score,
          source_method: "bseed_cofc_expiring",
          suggested_opener: `Your rental at ${addr} has a Certificate of Compliance expiring in ${daysLeft} days — inspectors flag dead limbs overhanging the structure and blocked egress paths. Tree trimming before the inspection removes that risk.`,
  ...
  } catch (e) { console.error("[tree] cofc expiring:", e); }
