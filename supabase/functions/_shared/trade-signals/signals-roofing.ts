...
export async function scanSignals(
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          address: addr, city: "Detroit", zip,
          signal_type: "roof_permit_upsell",
          signal_detail: `Historic District open violation: ${addr} (${a.historic_district ?? a.neighborhood ?? "Detroit"}) — roof/gutter/chimney cited. Scope: ${a.violation_scope ?? "not specified"}`,
          signal_date: a.intake_date ? a.intake_date.slice(0, 10) : new Date().toISOString().split("T")[0],
          score: BASE_SCORES.roof_permit_upsell + 1,
          source_method: "historic_district_violations",
          suggested_opener: `Your property at ${addr} has an open historic district violation for roof, gutter, or chimney condition. We specialize in historically-compliant repair work and can document compliance for your case closure.`,
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          ...
          signal_detail: `Macomb County new owner: ${addr} sold ${saleDate}. Home built ${yearBuilt > 0 ? yearBuilt : "pre-1990"} — new owner likely hasn't inspected a roof that's 35+ years old`,
          source_method: "macomb_county_parcel",
          suggested_opener: `Congrats on the new home at ${addr}! Homes built before 1990 often need a roofing inspection — shingles, flashing, and gutters wear out together. Free 20-min assessment?`,
          best_call_window: "Within 90 days of purchase",
          estimated_value: 12000,
          raw_source_data: { ...a },
        });
  ...
  } catch (e) { console.error("[roofing] Macomb County parcel:", e); }
