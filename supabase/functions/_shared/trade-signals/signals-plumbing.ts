...
export async function scanSignals(state = "MI", zipFilter?: string[]): Promise<RawSignal[]> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          address: addr, city: "Detroit", zip,
          signal_type: "cofc_plumbing_inspection",
          signal_detail: `CofC expires in ${daysLeft} days — BSEED inspects plumbing/water heater at renewal. ${addr} landlord must have all fixtures operational`,
          signal_date: today,
          score,
          source_method: "bseed_cofc_expiring",
          suggested_opener: `Your rental at ${addr} has a Certificate of Compliance expiring in ${daysLeft} days — inspectors check hot water, drainage, and no active leaks. We can do a pre-inspection plumbing walkthrough to catch problems before BSEED does.`,
  ...
  } catch (e) { console.error("[plumbing] cofc expiring:", e); }

  // Wayne County Parcel Sales — new owner + old home → galvanized pipe proxy
  try {
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          ...
          signal_detail: `Wayne County new owner: ${addr} sold ${saleDate}. Home built ${yearBuilt > 0 ? yearBuilt : "pre-1990"} — homes of that era commonly have galvanized pipe or original fixtures at end of life`,
          source_method: "wayne_county_parcel",
          suggested_opener: "Saw your home recently sold on Wayne County records — homes built before 1990 often have galvanized pipe or original fixtures at end of life. Free walkthrough this week?",
          best_call_window: "Within 90 days of purchase",
          estimated_value: 5000,
          raw_source_data: { ...a },
        });
  ...
  } catch (e) { console.error("[plumbing] Wayne County parcel:", e); }

  // Oakland County Parcel Sales — new owner + old home → galvanized pipe proxy
  try {
    const since180 = new Date(Date.now() - 180 * 86400_000).toISOString().slice(0, 10);
    const res = await fetch(
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          ...
          signal_type: "plumbing_permit_major",
          signal_date: saleDate, score: 6,
          signal_detail: `Oakland County new owner: ${addr} built ${yearBuilt > 0 ? yearBuilt : "pre-1990"} — older Oakland County homes commonly have original galvanized supply lines`,
          source_method: "oakland_county_parcel",
          suggested_opener: "Saw your home recently sold on Oakland County records — homes built before 1990 often have galvanized pipe or original fixtures at end of life. Free walkthrough this week?",
          best_call_window: "Within 90 days of purchase",
          estimated_value: 5000,
          raw_source_data: { ...a },
        });
  ...
  } catch (e) { console.error("[plumbing] Oakland County parcel:", e); }

  // Macomb County Parcel Sales — new owner + old home → galvanized pipe proxy
  try {
    const since180 = new Date(Date.now() - 180 * 86400_000).toISOString().slice(0, 10);
    const res = await fetch(
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          ...
          signal_type: "plumbing_permit_major",
          signal_date: saleDate, score: 6,
          signal_detail: `Macomb County new owner: ${addr} sold ${saleDate}. Home built ${yearBuilt > 0 ? yearBuilt : "pre-1990"} — homes of that era commonly have galvanized pipe or original fixtures at end of life`,
          source_method: "macomb_county_parcel",
          suggested_opener: "Saw your home recently sold on Macomb County records — homes built before 1990 often have galvanized pipe or original fixtures at end of life. Free walkthrough this week?",
          best_call_window: "Within 90 days of purchase",
          estimated_value: 5000,
          raw_source_data: { ...a },
        });
  ...
  } catch (e) { console.error("[plumbing] Macomb County parcel:", e); }
