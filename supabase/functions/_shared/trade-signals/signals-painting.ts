...
export async function scanSignals(state = "MI", zipFilter?: string[]): Promise<RawSignal[]> {
  ...
  // Historic District Violations — open siding/windows/paint violations per address
  try {
    const where = encodeURIComponent(
      `case_status = 'Open' AND (has_siding_walls_violation = 'True' OR has_windows_violation = 'True' OR has_paint_violation = 'True' OR has_doors_violation = 'True')`,
    );
    const res = await fetch(
      `https://services2.arcgis.com/qvkbeam7Wirps6zC/arcgis/rest/services/Historic_District_Violations/FeatureServer/0/query?where=${where}&outFields=address,zip_code,intake_date,violation_scope,historic_district,neighborhood,has_siding_walls_violation,has_windows_violation,has_paint_violation&resultRecordCount=40&orderByFields=ObjectId+DESC&f=json`,
      { headers: { "User-Agent": "DWA-TradeRadar/1.0 (matt@detroitwebagent.com)" } },
    );
    if (res.ok) {
      const d = await res.json();
      for (const feat of (d?.features ?? [])) {
        ...
        if (a.has_siding_walls_violation === "True") violations.push("siding/walls");
        if (a.has_windows_violation === "True") violations.push("windows");
        if (a.has_paint_violation === "True") violations.push("paint");
        signals.push({
          address: addr, city: "Detroit", zip,
          signal_type: "new_owner_exterior",
          signal_detail: `Historic District open violation: ${addr} (${a.historic_district ?? a.neighborhood ?? "Detroit"}) — ${violations.join(", ")} cited. Scope: ${a.violation_scope ?? "not specified"}`,
          signal_date: a.intake_date ? a.intake_date.slice(0, 10) : new Date().toISOString().split("T")[0],
          score: BASE_SCORES.new_owner_exterior + 1,
          source_method: "historic_district_violations",
          suggested_opener: `Your property at ${addr} has an open historic district violation for ${violations.join(" and ")}. We specialize in historically-compliant exterior work — siding, windows, and painting — and provide the documentation required for case closure.`,
          best_call_window: "Before next violation hearing — typically 30-60 days",
          estimated_value: 8000,
          raw_source_data: { addr, zip, scope: a.violation_scope, district: a.historic_district, violations, intake: a.intake_date },
        });
      }
    }
  ...
  try {
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          address: addr, city: "Detroit", zip,
          signal_type: "cofc_exterior_inspection",
          signal_detail: `CofC expires in ${daysLeft} days — BSEED inspects siding, paint peeling, and window condition at renewal. ${addr} exterior defects are common CofC failure points`,
          signal_date: today,
          score,
          source_method: "bseed_cofc_expiring",
          suggested_opener: `Your rental at ${addr} has a Certificate of Compliance expiring in ${daysLeft} days — BSEED commonly fails properties for peeling exterior paint, damaged siding, and broken windows. A pre-inspection exterior assessment ensures you pass.`,
  ...
  } catch (e) { console.error("[exterior] cofc expiring:", e); }

  // Wayne County Parcel Sales — new owner + old home → exterior refresh proxy
  try {
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          ...
          signal_detail: `Wayne County new owner: ${addr} sold ${saleDate}. Home built ${yearBuilt > 0 ? yearBuilt : "pre-1990"} — new owners of older Wayne County homes often discover outdated siding or peeling paint during the first season`,
          source_method: "wayne_county_parcel",
          suggested_opener: "New owners of older Wayne County homes often discover outdated siding or peeling paint during the first season — we offer a free exterior assessment.",
          best_call_window: "Within 120 days of purchase",
          estimated_value: 8000,
          raw_source_data: { ...a },
        });
  ...
  } catch (e) { console.error("[exterior] Wayne County parcel:", e); }

  // Oakland County Parcel Sales — new owner + old home → exterior refresh proxy
  try {
    const since180 = new Date(Date.now() - 180 * 86400_000).toISOString().slice(0, 10);
    const res = await fetch(
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          ...
          signal_type: "storm_siding_damage",
          signal_date: saleDate, score: 6,
          signal_detail: `Oakland County new owner: ${addr} built ${yearBuilt > 0 ? yearBuilt : "pre-1990"} — new owners of older Oakland County homes often discover outdated siding or peeling paint in the first season`,
          source_method: "oakland_county_parcel",
          suggested_opener: "New owners of older Oakland County homes often discover outdated siding or peeling paint during the first season — we offer a free exterior assessment.",
          best_call_window: "Within 120 days of purchase",
          estimated_value: 8000,
          raw_source_data: { ...a },
        });
  ...
  } catch (e) { console.error("[exterior] Oakland County parcel:", e); }

  // Macomb County Parcel Sales — new owner + old home → exterior refresh proxy
  try {
    const since180 = new Date(Date.now() - 180 * 86400_000).toISOString().slice(0, 10);
    const res = await fetch(
    ...
    if (res.ok) {
      ...
      for (const feat of (d?.features ?? [])) {
        ...
        signals.push({
          ...
          signal_type: "storm_siding_damage",
          signal_date: saleDate, score: 6,
          signal_detail: `Macomb County new owner: ${addr} sold ${saleDate}. Home built ${yearBuilt > 0 ? yearBuilt : "pre-1990"} — new owners of older Macomb County homes often discover outdated siding or peeling paint during the first season`,
          source_method: "macomb_county_parcel",
          suggested_opener: "New owners of older Macomb County homes often discover outdated siding or peeling paint during the first season — we offer a free exterior assessment.",
          best_call_window: "Within 120 days of purchase",
          estimated_value: 8000,
          raw_source_data: { ...a },
        });
  ...
  } catch (e) { console.error("[exterior] Macomb County parcel:", e); }
