...
export async function scanSignals(state = "MI", zipFilter?: string[]): Promise<RawSignal[]> {
  ...
  // OpenStreetMap Overpass — abandoned buildings in SE Michigan
  try {
    ...
    if (res.ok) {
      ...
      for (const el of (d?.elements ?? [])) {
        ...
        signals.push({
          ...
          signal_type: "demo_permit",
          signal_detail: `Abandoned ${buildingType} at ${addr}, ${city} — OpenStreetMap data shows unmaintained vacant structure needing demo or remediation`,
          signal_date: new Date().toISOString().split("T")[0],
          score: 7,
          source_method: "osm_abandoned_buildings",
          suggested_opener: `There's an abandoned ${buildingType} at ${addr} that may need full demolition and debris removal. We handle full teardown-to-cleanout with same-week scheduling and direct coordination with your city inspector.`,
          best_call_window: "Any time — long-standing vacant",
          estimated_value: buildingType.includes("apartment") || buildingType.includes("commercial") ? 15000 : 5000,
