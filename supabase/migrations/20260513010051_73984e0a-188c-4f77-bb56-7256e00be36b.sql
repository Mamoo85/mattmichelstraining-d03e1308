...
BEGIN
  ...
  PERFORM cron.unschedule('inventory-sentinel-4h') WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'inventory-sentinel-4h');
  PERFORM cron.schedule('inventory-sentinel-4h', '0 */4 * * *',
    format($job$SELECT net.http_post(url := %L, headers := %L::jsonb, body := '{}'::jsonb);$job$,
      v_url || '/functions/v1/inventory-sentinel', v_hdr));

  PERFORM cron.unschedule('inventory-daily-digest') WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'inventory-daily-digest');
  PERFORM cron.schedule('inventory-daily-digest', '0 13 * * *',
    format($job$SELECT net.http_post(url := %L, headers := %L::jsonb, body := '{}'::jsonb);$job$,
      v_url || '/functions/v1/inventory-daily-digest', v_hdr));
END $migration$;