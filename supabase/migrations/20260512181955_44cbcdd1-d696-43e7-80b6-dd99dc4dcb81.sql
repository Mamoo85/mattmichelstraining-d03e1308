...
BEGIN
  ...
  PERFORM cron.unschedule('missed-call-prospect-scanner-daily') WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'missed-call-prospect-scanner-daily');
  PERFORM cron.schedule('missed-call-prospect-scanner-daily', '0 12 * * *',
    format($job$SELECT net.http_post(url := %L, headers := %L::jsonb, body := '{}'::jsonb);$job$,
      v_url || '/functions/v1/missed-call-prospect-scanner', v_hdr));
END $migration$;