ALTER TABLE public.mortgage_radar_clients
  ADD COLUMN IF NOT EXISTS coverage_counties TEXT[] DEFAULT ARRAY[]::TEXT[],
  ADD COLUMN IF NOT EXISTS coverage_regions TEXT[] DEFAULT ARRAY[]::TEXT[];
...
BEGIN
  ...
  VALUES
    ('bb000002-0000-0002-0002-000000000002'::uuid,
  ...
  ON CONFLICT (id) DO UPDATE SET
    target_zip_codes = EXCLUDED.target_zip_codes,
    target_roles = EXCLUDED.target_roles,
    active = true;

  DELETE FROM public.address_validation_cache 
    WHERE pass = false AND cached_at >= NOW() - INTERVAL '2 days';

  UPDATE public.quarantine_history 
    SET hit_count = 0 
    WHERE last_reject_code IN ('validation_unavailable','validation_api_error','validation_exception');
END $$;