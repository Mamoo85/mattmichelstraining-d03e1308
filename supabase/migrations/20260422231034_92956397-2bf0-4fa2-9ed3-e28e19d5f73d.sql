...
BEGIN
  ...
  PERFORM public.safe_cron_schedule(
    'enrich-prospect-pool-hourly',
    '20 * * * *',
    format($job$SELECT net.http_post(url := %L, headers := %L::jsonb, body := jsonb_build_object('trigger','cron','limit',25));$job$,
      v_url || '/functions/v1/enrich-prospect-pool', v_hdr)
  );
END $migration$;