// STRIPE WEBHOOK — No npm:stripe (signature verified with native crypto)
...
async function sendEmail(to: string, subject: string, html: string, bcc = "matthewmichels4@gmail.com") {
  if (!RESEND_API_KEY) return;
  await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: { Authorization: `Bearer ${RESEND_API_KEY}`, "Content-Type": "application/json" },
    body: JSON.stringify({ from: "Matt Michels <matt@mattmichelstraining.com>", to: [to], bcc: [bcc], subject, html }),
  });
}

async function notifyMatt(subject: string, html: string) {
  await sendEmail("matt@mattmichelstraining.com", subject, html, "matthewmichels4@gmail.com");
}
...
<div style="max-width:600px;margin:0 auto;padding:32px 16px;">
  ...
  <div style="text-align:center;margin-top:24px;">
    <p style="color:#475569;font-size:12px;">Matt Michels · Detroit Web Agency · <a href="tel:+13139921219" style="color:#00d4ff;">(313) 992-1219</a></p>
      ...
      // ─── Talent Radar (staffing agency product) ───────────────────────────────
      if (meta.type === "talent_radar_sheet" || meta.type === "talent_radar_subscription") {
        const email = meta.email || customerEmail;
        try {
          if (email) {
            const verticals = meta.target_verticals
              ? meta.target_verticals.split(",").map((v: string) => v.trim()).filter(Boolean)
              : ["trades"];
            const { data: insertedClient, error: insertErr } = await (sb.from as any)("talent_radar_clients").upsert({
              company_name: meta.company_name || email,
              owner_email: email,
              owner_phone: meta.owner_phone || null,
              stripe_customer_id: session.customer as string || null,
              stripe_subscription_id: session.subscription as string || null,
              plan: meta.plan || "sheet",
              active: true,
              target_verticals: verticals,
            }, { onConflict: "owner_email" }).select("id, dashboard_token").single();
            if (insertErr) throw new Error(`talent_radar_clients upsert: ${insertErr.message}`);
            const dashboardToken = insertedClient?.dashboard_token;
            const isSheet = meta.type === "talent_radar_sheet";
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;">
  <tr><td style="background:linear-gradient(135deg,#0a1628 0%,#0d1f3c 100%);padding:36px 28px 28px;border-radius:16px 16px 0 0;border-bottom:3px solid #00d4ff;text-align:center;">
    <p style="margin:0;color:#00d4ff;font-size:11px;font-weight:800;letter-spacing:4px;text-transform:uppercase;">🎯 Talent Radar</p>
    <p style="margin:12px 0 0;color:#fff;font-size:26px;font-weight:800;">${isSheet ? "Your Sheet Is Ready" : "You're In — Talent Radar Is Live"}</p>
    <p style="margin:6px 0 0;color:#94a3b8;font-size:14px;">${isSheet ? "10 new-licensee contacts, ready to download." : "New license intelligence, delivered daily."}</p>
  </td></tr>
  <tr><td style="background:#fff;padding:28px;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;">
    <p style="color:#1e293b;font-size:15px;line-height:1.8;margin:0 0 16px;">Hey${meta.company_name ? ` ${meta.company_name}` : ""} —</p>
    <p style="color:#475569;font-size:15px;line-height:1.8;margin:0 0 20px;">${isSheet
      ? "Your 10-name intelligence sheet is queued and will be ready in your dashboard within minutes. Each contact includes name, license number, city, phone, email, and availability score."
      : `Welcome to Talent Radar <strong>${meta.plan === "agency" ? "Agency" : meta.plan === "pro" ? "Pro" : "Starter"}</strong>. Every morning at 7am we scan Michigan's licensing database and surface new licensees in your target vertical — people your competitors haven't called yet.`
    }</p>
    <p style="color:#475569;font-size:14px;line-height:1.8;margin:0 0 20px;"><strong>What's inside each contact:</strong> Full name · License type &amp; number · City · Phone (when available) · Email · LinkedIn · Availability score (1-10) · AI hiring recommendation</p>
    <p style="color:#475569;font-size:14px;line-height:1.8;margin:0 0 8px;"><strong>ROI math:</strong> One CNA placement = $6,700–$10,400 in placement fees. One sheet costs $250. That's a 27–42x return on a single placement.</p>
    ${dashboardToken ? `<table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:20px 0 8px;"><a href="https://detroitwebagent.com/my-talent-radar?token=${dashboardToken}" style="display:inline-block;padding:14px 32px;background:#00d4ff;color:#0a1628;font-weight:800;font-size:15px;border-radius:8px;text-decoration:none;">Open Dashboard →</a></td></tr></table>` : ""}
  </td></tr>
  <tr><td style="padding:20px 28px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 16px 16px;background:#0a1628;">
    <p style="margin:0;font-size:14px;font-weight:700;color:#fff;">Matt Michels</p>
    <p style="margin:2px 0 0;font-size:12px;color:#94a3b8;">Detroit Web Agency · <a href="tel:+13139921219" style="color:#00d4ff;text-decoration:none;">(313) 992-1219</a> · <a href="mailto:matt@detroitwebagency.com" style="color:#00d4ff;text-decoration:none;">Unsubscribe</a></p>
  </td></tr>
</table></td></tr></table>
</body></html>`;
              await dwaEmail(email,
                isSheet ? "🎯 Talent Radar — Your 10-Name Sheet Is Ready" : "🎯 Talent Radar Is Live — New License Intelligence Active",
                welcomeHtml
              );
              await notifyMatt(
                `💰 New Talent Radar Client — ${meta.company_name || email} (${isSheet ? "$250 sheet" : `$${meta.plan === "agency" ? "1499" : meta.plan === "pro" ? "799" : "499"}/mo`})`,
                `<p><strong>${meta.company_name || email}</strong><br>Email: ${email}<br>Phone: ${meta.owner_phone || "n/a"}<br>Plan: ${meta.plan}<br>Verticals: ${meta.target_verticals || "trades"}</p>`
              );
            }
          }
        } catch (e) {
          console.error("[WEBHOOK] talent_radar error:", e);
          await notifyMatt(
            `🚨 Talent Radar provision FAILED — ${email || "unknown"} paid but not activated`,
            `<p>Error: ${e instanceof Error ? e.message : String(e)}</p><p>Stripe session: ${session.id}</p>`
          ).catch(() => {});
          return new Response(JSON.stringify({ error: "provisioning failed" }), { status: 500 });
        }
...
<p style="margin:0;color:#64748b;font-size:13px">Add more domains any time from your dashboard. White-label reports available — reply to this email to discuss.</p>`,
    ...
    if (event.type === "checkout.session.expired") {
      ...
      const expMeta = expired.metadata || {};
      const expEmail = expMeta.email || expired.customer_details?.email || null;
      const instantTypes = ["website_audit", "gbp_post_pack", "competitor_report"];
      if (expMeta.type && instantTypes.includes(expMeta.type) && expEmail) {
        const { count } = await sb.from("cart_abandonments").select("*", { count: "exact", head: true }).eq("stripe_session_id", expired.id);
        if (!count) {
          await sb.from("cart_abandonments").insert({
            email: expEmail, product_type: expMeta.type, stripe_session_id: expired.id,
            cart_value: expMeta.price ? parseFloat(expMeta.price) : 49, metadata: expMeta,
          });
        }
