    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          ...
          unit_amount: 3900,
          product_data: { name: "AI Appointment Reminders", description: "Automated SMS appointment reminders to reduce no-shows." },
        },
        quantity: 1,
      }],
      metadata: { type: "appointment_reminder_subscription", email, name: name || "", businessName, phone: phone || "" },
      success_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/appointment-reminders?status=success`,
      cancel_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/appointment-reminders`,
    });
