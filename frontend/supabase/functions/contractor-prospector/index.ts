...
function buildSniperEmail(
  ...
  const [cityShort] = extractCityState(city);
  const tradeLabel = trade.replace(/ contractor$/i, "").toLowerCase();
  const flaw = issues[0] || "not showing up on the first page of Google";

  if (offer.offer === "leads") {
    return {
      subject: `${cityShort} ${tradeLabel} leads — open right now`,
      body: `Hey —\n\nI've got verified ${tradeLabel} homeowner leads in ${cityShort} sitting unclaimed right now. Exclusive — one contractor per lead, no bidding, no shared lists.\n\n$399/month flat. No contract. Cancel any time.\n\nSee what's open: https://detroitwebagent.com/contractor-leads\n\nOr text me: (313) 992-1219 — Matt, Grosse Pointe`,
    };
  }

  if (offer.offer === "missed_call") {
    return {
      subject: `${businessName} — missed calls going to your competitors`,
      body: `Hey —\n\nWhen someone calls ${businessName} and hits voicemail, they call the next ${tradeLabel} on Google. First shop to text back gets the job.\n\nMissed-Call Catch auto-texts every missed caller within 30 seconds. $99/month, 7-day free trial.\n\nSee it: https://detroitwebagent.com/missed-call-catch\n\nOr text me: (313) 992-1219 — Matt, Grosse Pointe`,
    };
  }

  // Default: web design
  return {
    subject: `${businessName} — quick look at your site`,
    body: `Hey —\n\nRan a quick check on ${businessName} — spotted ${flaw}. That's probably sending 2–3 jobs a week to whoever shows up first on Google.\n\nI build sites for ${tradeLabel}s in ${cityShort} that rank and convert. $499 to build, $49/month for hosting, SEO, and unlimited edits.\n\nhttps://detroitwebagent.com/web-design-services\n\nOr text me: (313) 992-1219 — Matt, Grosse Pointe`,
  };
}
