...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: "usd", recurring: { interval: "month" }, unit_amount: 2900, product_data: { name: "AI Google Q\&A Manager — $29/month", description: "Auto-answer questions on your Google Business Profile." } }, quantity: 1 }],
      metadata: { type: "google_qa_subscription", email, name: name || "", businessName },
      success_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/google-qa?status=success`,
      cancel_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/google-qa`,
    });
