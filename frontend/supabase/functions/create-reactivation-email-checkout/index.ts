...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: "usd", recurring: { interval: "month" }, unit_amount: 3900, product_data: { name: "AI Customer Reactivation Email — $39/month", description: "Monthly personalized emails to your inactive customers." } }, quantity: 1 }],
      metadata: { type: "reactivation_email_subscription", email, name: name || "", businessName, industry: industry || "" },
      success_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/reactivation-emails?status=success`,
      cancel_url: `${req.headers.get("origin") || "https://www.detroitwebagent.com"}/reactivation-emails`,
    });
