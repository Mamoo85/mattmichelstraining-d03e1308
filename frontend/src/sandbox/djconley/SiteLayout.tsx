...
// Mirrors djconley.com top navigation 1:1 (no internal "Command Center" link —
// the admin is reached via the private link we send the owner).
const NAV = [
    ...
    <div className="min-h-screen bg-[#e9e9e9] text-[#555]" style={{ fontFamily: "Arial, Helvetica, sans-serif" }}>
      <header className={`${isHome ? "absolute" : "sticky bg-black shadow-sm"} top-0 z-40 w-full text-white`}>
        <div className={`mx-auto flex h-[94px] max-w-[1200px] items-center px-6 md:h-[156px] ${isHome ? "justify-center" : "justify-between"}`}>
          ...
          <nav className={`${isHome ? "hidden" : "hidden xl:flex"} items-center gap-4 text-[11px] font-semibold uppercase tracking-[0.14em]`}>
            {NAV.map((n) => (
              <NavLink key={n.to} to={djPath(n.to)} end={n.end} className={({ isActive }) => isActive ? "text-white" : "text-white/80 hover:text-white"}>
                {n.label}
              </NavLink>
            ))}
          </nav>
        ...
        {open && (
          <div className="border-t border-white/15 bg-black px-6 py-4">
            <div className="mx-auto grid max-w-[420px] gap-1">
              {NAV.map((n) => (
                <NavLink key={n.to} to={djPath(n.to)} end={n.end} onClick={() => setOpen(false)} className="px-2 py-3 text-sm uppercase tracking-[0.16em] text-white/90">
                  {n.label}
                </NavLink>
              ))}
