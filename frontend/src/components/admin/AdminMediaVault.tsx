          ...
          for (const item of bucketFiles) {
            if (item.id === null) {
              ...
              for (const sub of subFiles) {
                ...
                let resp: Response;
                try { resp = await fetch(publicUrl, { signal: ac1.signal }); } catch { clearTimeout(t1); continue; }
                clearTimeout(t1);
                if (!resp.ok) continue;
              ...
              let resp: Response;
              try { resp = await fetch(publicUrl, { signal: ac2.signal }); } catch { clearTimeout(t2); continue; }
              clearTimeout(t2);
              if (!resp.ok) continue;
              const blob = await resp.blob();
