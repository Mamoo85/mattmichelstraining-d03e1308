...
const AdminDwaAdStudio = lazy(() => import("@/components/dwa-admin/AdminDwaAdStudio"));
...
const AdminSDRClients = lazy(() => import("@/components/dwa-admin/AdminSDRClients"));
const ClientOnboardingGuide = lazy(() => import("@/components/dwa-admin/ClientOnboardingGuide"));

type Tab =
  | "today" | "ai-command"
  | "dwa-overview" | "revenue" | "leads-e2e" | "prospect-tracker" | "agent-toolkit" | "pipeline-velocity"
  | "command-center" | "sms-inbox" | "sms-drafts" | "call-list" | "linkedin-blitz" | "ad-launcher" | "dwa-ad-studio" | "agency-outreach" | "dead-leads" | "fax-drip" | "postcard-drip" | "sms-sniper" | "dead-lead-ad-studio" | "wave5-outreach"
  ...
  | "client-onboarding"
  | "sales-hub" | "buyer-radar-qa" | "mortgage-radar" | "strategy-mode" | "enrichment-audit" | "enrichment-health" | "enrichment-debug" | "reply-inbox" | "client-attribution" | "marketing-tools" | "djconley" | "proposals" | "manual-onboarding" | "suppression-lists" | "system-audit" | "demo-pipeline" | "trial-funnel" | "email-preflight" | "sdr-clients";
...
const GROUPS: SidebarGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "sdr-clients",           label: "🤖 SDR Clients (sell hunters)" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "dwa-ad-studio",     label: "🎬 Ad Background Studio" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "visitor-intel",     label: "SiteRadar" },
      { id: "buyer-radar-qa",    label: "Buyer Radar" },
    ...
    <div className="min-h-screen bg-[#0a1628] text-white flex w-full">
      ...
      <main className="flex-1 min-w-0 overflow-x-hidden">
        <header className="h-14 sticky top-0 z-30 bg-[#0a1628]/95 backdrop-blur border-b border-white/10 flex items-center justify-between px-4 sm:px-6">
          ...
          {/* Quick links to new e-commerce pages */}
          <div className="hidden sm:flex items-center gap-2">
            <button onClick={() => navigate("/dwa-admin/services")} className="text-xs px-2 py-1 rounded bg-white/10 hover:bg-white/20 text-white/80 transition-colors">🛒 Services</button>
            <button onClick={() => navigate("/dwa-admin/shorts")} className="text-xs px-2 py-1 rounded bg-green-500/20 hover:bg-green-500/30 text-green-300 transition-colors">▶ Shorts</button>
            <button onClick={() => navigate("/dwa-admin/marketing-hub")} className="text-xs px-2 py-1 rounded bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 transition-colors">📣 Marketing</button>
          </div>
        </header>
        ...
        <div className="px-3 sm:px-6 py-6">
          ...
          {activeTab === "dwa-ad-studio"      && <Suspense fallback={lazyFallback("Ad Background Studio")}><AdminDwaAdStudio /></Suspense>}
          ...
          {activeTab === "client-onboarding" && <Suspense fallback={lazyFallback("Client Onboarding")}><ClientOnboardingGuide /></Suspense>}
          ...
          {activeTab === "sdr-clients"     && <Suspense fallback={lazyFallback("SDR Clients")}><AdminSDRClients /></Suspense>}
