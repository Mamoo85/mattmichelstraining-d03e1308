...
| Check | Table | Flag if |
|-------|-------|---------|
| Scanner dispatch stalled | `hire_REDACTED` | `last_completed_at > now() - interval '30 minutes'` for any source |
| Legacy scanner silent | `hire_alert_runs` | No row with `run_at > now() - interval '25 hours'` |
| Source workers down | `hire_REDACTED` | `status = 'processing'` for > 45 min — lock is stuck |
...
- For TechAlert: always check `hire_REDACTED` in addition to `hire_alert_runs` — the checkpoint table is ground truth for per-source worker health.
