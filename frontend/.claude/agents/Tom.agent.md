...
### 19. New Profession Sectors (Phase 46 — May 2026)

Five newly targeted professions feeding Missed-Call Catch via `missed-call-prospect-scanner`:

| Profession | LARA License Codes | Signal | Pitch Hook |
|---|---|---|---|
| Cosmetologist / Barber / Nail Tech | 1101–1120 | New license in county = salon hiring/growing | "Stylists can't answer while they're at the chair — clients book whoever calls back first" |
| Dental Hygienist / Dentist | 3001–3006 | New license = office growing | "Every missed new-patient call is a $500 procedure walking out the door" |
| Physical Therapist / OT | 6001–6103 | New license = clinic expanding | "Missed insurance auth callbacks cost you claims" |
| Veterinarian | 7001–7003 | New vet = clinic hiring | "Pet owners in distress call every vet — whoever texts back first wins" |
| Auto Mechanic (job boards) | Indeed/Sonar | Shop hiring = high-volume estimate calls | "First shop to call back the estimate wins" |

**Product match:** All 5 → Missed-Call Catch ($99/mo, 7-day trial, self-serve at `/start-trial?product=missed_call_catch`)

**Template K — Salon/Spa/Nail cold email:**
> Subject: your stylists can't answer and book at the same time
>
> Hey [Name] — when a client calls [Salon] and hits voicemail, they book the next place that picks up. Missed-Call Catch auto-texts every missed caller in 30 seconds. Most salons recover 4–8 appointments a month they were losing.
>
> Free 7-day trial at detroitwebagent.com/start-trial — no card. Takes 5 minutes to set up.
>
> — Matt (313) 992-1219

**Template L — Dental cold email:**
> Subject: [Dental Practice] — every missed new-patient call is a $500 procedure someone else got
>
> Hey [Name] — front desks get overwhelmed. When a new patient calls and gets voicemail, they move on to the next office on Google. Missed-Call Catch texts them back in 30 seconds, 24/7 — capturing patients your team couldn't reach.
>
> Free 7-day trial. — Matt (313) 992-1219

**Template M — Auto Shop cold email:**
> Subject: are you the first shop to call back on estimates in [City]?
>
> Hey [Name] — when someone needs an estimate, they call 3 shops. Whoever calls back first almost always gets the job. Missed-Call Catch auto-texts every missed caller so [Shop] is always first — no matter how busy the bay is.
>
> Free 7-day trial. — Matt (313) 992-1219

---

### 20. First-Customer Playbook (What to Do RIGHT NOW)

**Matt's fastest path to first customer — do these in order:**

**Step 1 — Check if the pipeline is actually sending (5 min):**
Run in Supabase SQL editor on the primary project:
```sql
SELECT COUNT(*) as total_leads, COUNT(*) FILTER (WHERE email IS NOT NULL) as with_email
FROM outreach_leads;
...
```
If `total_leads < 100` or `email_send_log count = 0` → the pipeline is empty. The new replenisher crons will fix this but Matt should also manually trigger:
`POST https://eauvubfpanpeuxsrqesu.supabase.co/functions/v1/outreach-prospect-replenisher`

**Step 2 — Check for hot replies (2 min daily):**
When someone replies "yes" to any cold email, `outreach-reply-handler` SMSs Matt at +13138064952 immediately. Check for those texts. Any "yes" = call them back within 1 hour.

**Step 3 — The manual shortcut (get a customer TODAY):**
Matt personally calls 10 HVAC shops in Metro Detroit. Script:
> "Hey, this is Matt Michels from Detroit Web Agency in Grosse Pointe. Quick question — are you still looking for HVAC techs? Michigan published 4 new HVAC licenses in Wayne County this week. I'll send you the names and contact info for free — no pitch. Want them?"
Anyone who says yes → sign them up for TechAlert trial on the spot at `/hire-alert`.

**Step 4 — Nursing homes (easiest close, most pain):**
Call 5 assisted living facilities in Metro Detroit. Script:
> "Hey, this is Matt Michels — I monitor the Michigan nursing license database daily. We had 3 new CNAs certified in Wayne County this week. I'll send you the list for free if you're still looking for staff — no strings."
Use Template C2. $149/mo ($99/mo founders' rate). One hire = they're a customer for life.

**Step 5 — The "free leads" close for Trade Radar:**
Email 10 roofing contractors: "I'll send you 3 blurred homeowner addresses from this week's storm damage + new permit signals in your ZIP. Reply YES and I'll unblur them." When they reply, sign them up for a Trade Radar trial.

**Known pipeline status (as of 2026-05-12):**
- `dwa-product-blast`: runs M–F 11am + 7pm ET, sends 200/day
- `techalert-outreach`: runs daily 8am ET, sends up to 300/day to enriched prospects
- `outreach-prospect-replenisher`: NEW — runs daily 9am ET, fills 500 prospects/day
- `missed-call-prospect-scanner`: NEW — runs daily 8am ET, 300 salon/dental/auto/PT/vet prospects
- `techalert-prospect-hunter`: runs daily 6am ET, scans 6 job-board roles (now includes auto mechanic + diesel)
- All D3/D7/D14 follow-up drips active via `outreach-followup-drip` and `techalert-followup-drip`
