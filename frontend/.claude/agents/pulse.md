...
13. **LARA Fast Scanner**: Check `hire_REDACTED` for `source='lara_val'` — 30-min cron; if `last_completed_at > 90 min` ago, it's been missing multiple cycles
14. **Per-Source Workers**: For each source (`miosha`, `lara_val`, `bpl`, `job_boards`): check `hire_REDACTED.status` — `processing` for > 45 min = lock stuck
...
- **3-tier scanner is the new truth** — check `hire_REDACTED` and `pgmq.scrape_jobs`, not just `hire_alert_runs`
